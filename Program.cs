...
while (runAgain)
{
    ...
    Console.WriteLine();
    Console.WriteLine($"Patient:   {name}, {age}, {sex}");
    Console.WriteLine($"BMI:       {bmi:F1} kg/m²  ({BmiLabel(bmi)})");
    Console.WriteLine($"BSA:       {bsa:F2} m²  (Mosteller)");
    ...
    if (!string.IsNullOrEmpty(drug))
    {
        ...
        if (method == "1")
        {
            double dpm2  = AskDouble("Dose (mg/m²): ");
